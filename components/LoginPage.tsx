...
export default function LoginPage() {
  ...
  const handleDemoLogin = async (accNum: number) => {
    setError('');
    setLoading(true);
    const demoEmail = accNum === 1 ? 'demo1@example.com' : 'demo2@example.com';
    ...
    try {
      ...
      if (result.error) {
        setError(result.error.message || `Demo account ${accNum} could not be found.`);
      } else {
        router.push('/dashboard');
        router.refresh();
      }
